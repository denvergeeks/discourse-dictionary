  ...
  api.renderAfterWrapperOutlet(
    ...
    class extends Component {
      static shouldRender(args) {
        if (args.post?.post_number !== 1) return false;
        if (!currentUser) return false;
        const minTrustLevel = siteSettings?.discourse_dictionary_min_trust_level || 0;
        if (currentUser.trust_level < minTrustLevel) return false;
        return true;
      }
        ...
        dialog.dialog({
          ...
          buttons: [
            { label: "Cancel", class: "btn-default" },
            {
              label: "Add Meaning",
              class: "btn-primary",
              action: () => this.handleAddMeaning(post)
            }
          ]
        });
      };
      ...
      handleAddMeaning = (post) => {
        ...
        ajax("/discourse-dictionary/add_meaning", {
          type: "POST",
          data: { word, post_id: post.id }
        })
          .then((result) => {
            dialog.alert({
              message: htmlSafe(
                result.success
                  ? result.message || "Dictionary meaning added successfully!"
                  : result.message || "Failed to add dictionary meaning"
              ),
